// src/repositories/rule.repository.js
...
export const getRulesByAsset = async (ctx, asset) => {
  const { Rule } = ctx.models;
  ...
  return Rule.find({
    isActive: true,
    $or: [{ assetIds: asset._id }, { assetTypeIds: asset.assetTypeId }],
  }).lean();
};
